...
@section('content')
    <div class="">
        ...
        <form action="{{ route('post.update',$post->slug) }}" method="POST" enctype="multipart/form-data" class="flex flex-col max-w-full mx-auto my-5 border p-5 rounded shadow-lg">
            @csrf
            @method('PATCH')
            <div class=" space-y-2 mb-3">
                <label for="featured_image" class=" text-gray-400 font-bold">Feature Image</label>
                <div class=" mb-3">
                    <img src="{{ asset('storage/'.$post->featured_image) }}" class=" w-72 rounded-md">
                </div>
            ...
            <div class=" space-y-2 mb-3">
                ...
                <input type="text" id="title" name="title"
                    class="block w-full bg-gray-100 text-gray-900 border-0 rounded-md p-2 mb-4 focus:bg-gray-200 focus:outline-none focus:ring-1 focus:ring-blue-500 transition ease-in-out duration-150"
                    placeholder="Enter category title" value="{{ old('title',$post->title) }}">
                    @error('title')
                    <small class=" text-red-500">{{ $message }}</small>
                @enderror
            </div>
            ...
            <div class=" space-y-2 mb-3">
                ...
                <textarea  id="content" name="content" rows="4"
                    class="w-full h-32 bg-gray-100 text-gray-900 border-0 rounded-md p-2 mb-4 focus:bg-gray-200 focus:outline-none focus:ring-1 focus:ring-blue-500 transition ease-in-out duration-150"
